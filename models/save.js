...
class Save {
    ...
    for (let line of saveDetailStreamLines) {
      ...
      if (line.length > 1) {
        ...
        if (key.includes("dependencies[")) {
          this.saveDetail.dependencies_list.push(new Dependencies(value));
        }
      }
    }
  }
  gameDetailLoad() {
    ...
    const ContainerTagFilter = (lines, startSlice, endSlice) => {
      let lineIndex = 0;
      ...
      for (let line of lines.slice(startSlice, endSlice)) {
        ...
        if (line.length > 3 && line[3].includes("{")) {
          ...
          lineContainerItem.startValueTag = line[0];
          lineContainerItem.startValueTagTitle = line[2];
    ...
    let tagList = [];
    let tagResult = null;
    lineContainers.forEach((element) => {
      tagList.push(element.startValueTag);
      //console.log(element.startValueTag);
      tagResult = tagList.reduce((acc, value) => {
